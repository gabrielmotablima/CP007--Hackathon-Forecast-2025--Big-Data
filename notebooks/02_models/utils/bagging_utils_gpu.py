...
def objective(trial, hyperparam_ranges, tscv, x_data, columns_to_use, preprocessing, random_state):
    ...
    def suggest_param(trial, name, params):
        is_float = any(isinstance(p, float) for p in params)
        if is_float:
    ...
    # Suggest hyperparameters for the BaggingRegressor
    bagging_params = {k: suggest_param(trial, f'bagging_{k}', v) for k, v in hyperparam_ranges['BAGGING'].items()}
    
    # Suggest hyperparameters for the base XGBoost model
    xgb_params = {k: suggest_param(trial, f'xgb_{k}', v) for k, v in hyperparam_ranges['XGBOOST'].items()}

    # Instantiate the base estimator
    base_estimator = XGBRegressor(**xgb_params, device='cuda', tree_method='hist', random_state=random_state)
    
    # Instantiate the Bagging Regressor with the base estimator
    ...
    for idx, (train_idx, val_idx) in enumerate(tscv.split(X_train), 1):
        ...
        ttr = TransformedTargetRegressor(regressor=bagging_model, func=np.log1p, inverse_func=np.expm1)
